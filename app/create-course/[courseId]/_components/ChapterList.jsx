                ...
                {course?.courseOutput?.chapters.map((chapter, index) => (
                    <div className='border p-5 rounded-lg mb-2 flex items-center justify-between' key = {index}>
                        <div className='flex gap-5 items-center' key={index}>
                            <h2 className='bg-[#875bf9] flex-none h-10 w-10 text-white  rounded-full text-center p-2'>{index + 1}</h2>
                            <div>
                                ...
                                <p className='flex gap-2 text-[#875bf9] items-center'> <HiOutlineClock /> {chapter?.duration}</p>
                            </div>
                        </div>
