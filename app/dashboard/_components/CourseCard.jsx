...
function CourseCard({course,refreshData,displayUser=false}) {
  ...
  return (
    <div className='shadow-sm rounded-lg  border p-2
         cursor-pointer mt-4 hover:border-[#875bf9] '>

          <Link href={'/course/'+course?.courseId}>
        
        <Image src={course?.courseBanner} alt='image' width={300} height={200} 
        ...
        <div className='p-2'>
            ...
            <div className='flex items-center justify-between'>
                ...
                p-1 bg-purple-50 text-[#875bf9] text-sm rounded-sm'> 
                <HiOutlineBookOpen /> {course?.courseOutput?.noOfChapters}  Chapters</h2>
                <h2 className='text-sm bg-purple-50 text-[#875bf9] p-1 rounded-sm'>{course?.level}</h2>
            </div>

          {displayUser&& <div className='flex gap-2 items-center mt-2'>
              <Image src={course?.userProfileImage} alt='img' width={35} height ={35} 
